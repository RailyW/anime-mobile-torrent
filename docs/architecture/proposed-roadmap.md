...
12. 章节完整分页、批量标记到第 N 话和批量管理仍是后续工作。
...
1. Flutter 工程骨架。
2. Bangumi 登录、当前用户信息、搜索、条目详情、我的动画收藏分页列表、单条收藏读写和动画本篇章节状态同步。
...
复杂过滤、章节完整分页、批量进度操作、RSS 自动检查调度、外部客户端兼容性清单、公共目录导出和内置下载器可以在基础闭环跑通后逐步补齐。
