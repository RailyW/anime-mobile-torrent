...
7. 后台常驻使用用户显式开启的 Android Foreground Service，当前提供持续通知、低频心跳、通知点击默认回到后台页、新订阅命中时通知直达 DMHY 搜索、重复命中回后台摘要页、通知查看后台按钮、通知停止按钮、DMHY 搜索结果一键保存订阅、DMHY 订阅低频自动检查、新命中识别、前台最近摘要刷新、自动检查摘要复制和订阅结果回流 DMHY 搜索。
...
5. 已新增 `lib/features/subscriptions`，支持从后台订阅面板或 DMHY 搜索结果页保存 DMHY RSS 订阅关键词、选择动画分类或全站范围、手动检查 RSS、后台低频自动检查、新命中识别，并在前台展示、刷新和复制最近自动检查摘要与失败原因。
...
8. 已支持从订阅关键词、手动检查结果和带最新命中关键词的自动检查摘要跳转到 DMHY 搜索页，并保留动画分类或全站搜索范围；后台页也可以复制最近自动检查摘要，便于真实设备排查订阅心跳状态。也支持从 DMHY 搜索结果页反向保存当前搜索词为订阅关键词，后续 `.torrent` 下载仍由用户在 DMHY 页显式触发。
...
9. 已提供 DMHY RSS 订阅关键词保存、DMHY 搜索结果一键订阅、手动检查、后台低频自动检查、新命中识别、最近自动检查摘要入口、自动检查摘要复制入口、订阅关键词/检查结果回流 DMHY 搜索入口、后台通知回流入口、通知查看后台按钮，以及新订阅命中通知直达 DMHY 搜索入口。
