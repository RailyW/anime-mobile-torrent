...
4. Torrent 交接模块只负责 magnet 打开、magnet 复制、`.torrent` 种子文件下载、外部客户端直开、分享兜底和用户主动导出；当前已通过成熟 Flutter 插件落地直开与分享双路径，并通过 Android 系统文档创建器提供手动导入兜底。
5. BT 视频内容下载由用户手机自己的外部 BT 客户端负责，APP 不管理下载进度、暂停恢复、做种、限速和下载目录。
6. 播放模块首期可以只调起系统/第三方播放器；如果视频由外部客户端下载完成，APP 需要用户手动选择本地视频后才能播放。
7. 后台常驻使用用户显式开启的 Android Foreground Service，当前提供持续通知、低频心跳、通知点击默认回到后台页、新订阅命中时通知直达带来源提示的 DMHY 搜索、重复命中回后台摘要页、新订阅命中通知查看资源按钮、通知查看后台按钮、通知停止按钮、DMHY 搜索结果一键保存订阅、DMHY 订阅低频自动检查、前台立即执行后台检查规则、新命中识别、前台最近摘要刷新、自动检查摘要复制和订阅结果回流 DMHY 搜索。
...
1. 已接入可配置 OAuth 登录，使用 `webview_flutter` 打开 Bangumi `/oauth/authorize` 并截获 `https://bgm.tv/oauth/<redirect_uri>?code=...` 代理回调，使用 Dio 表单调用 `/oauth/access_token`，并提供本机 OAuth client 设置页；用户可以保存自己的 client id、client secret、redirect URI 和 scopes，保存或清除配置时会清理旧 token，过期 token 缺少 refresh token 时会自动清理本地凭据，redirect URI 必须使用当前默认的 `com.railyw.anime_mobile_torrent` scheme，移动端请求默认不传 `scope` 参数，实际权限以 Bangumi 开发者后台勾选项为准。
...
2. 当前 Bangumi OAuth 后台 callback 仍配置为自定义 scheme `com.railyw.anime_mobile_torrent://oauth/bangumi`，但移动端会在 WebView 中截获 Bangumi 生成的 HTTPS 代理回调；旧版单斜杠配置会在读取时归一化，发布前可继续评估后端 token broker 或更正式的 App Links/中转页方案。
...
5. 已支持按用户点击解析 DMHY 详情页 `.torrent` 链接，并把种子文件下载到 APP 专属文档目录的 `dmhy_torrents` 子目录、写入最近种子记录后优先直开外部 BT 客户端，直开失败时自动降级到系统分享面板；交接成功提示可带 DMHY 来源语境回流播放页，由用户在外部 BT 客户端完成视频下载后手动选择本地视频；交接失败或种子下载异常时可从提示条复制 RSS magnet 继续手动处理。
...
10. 已支持前台 DMHY 资源按字幕组、分辨率、片源、封装格式、视频编码、字幕说明、字幕语言、大小区间、最小种子数和排除关键词筛选，并支持把字幕组保存为本机偏好后在后续搜索结果中自动套用；筛选只作用于已加载结果，不触发额外 DMHY 请求。
11. 已支持在 DMHY 搜索结果页把当前关键词和动画分类/全站范围保存为订阅关键词，后续可由后台页手动检查或前台服务低频自动检查。
12. 已为 DMHY RSS 搜索、详情页解析和 `.torrent` 文件下载请求加入 429 `Retry-After` 退避重试一次，所有动作仍由用户显式搜索或点击触发。

待确认：

1. 是否要加入 Anime Garden 作为非官方备用源。
...
6. 已通过 Android Storage Access Framework `ACTION_CREATE_DOCUMENT` 支持把最近种子导出到用户选择的位置，供外部 BT 客户端手动导入；该能力不申请全文件访问权限，也不保存外部目录路径。
7. 已在种子交接页补充外部 BT 客户端兼容自检、失败处理和“不下载 BT 视频内容”的边界说明。
8. 已新增 Android MethodChannel，通过 PackageManager resolver 查询当前设备是否存在可处理 magnet、`.torrent` 直开和 `.torrent` 分享导入的外部客户端，并在种子交接页和 DMHY 资源卡片展示检测结果、候选应用名称和包名。
9. 已在种子交接页新增本机真实设备兼容实测记录，用户可以手动标记直开成功、分享成功、导出后手动导入成功、magnet 兜底成功或交接失败，并把最近记录保存在本机；如果记录错误，可以删除单条实测记录而不清空其他样本。
10. 已在种子交接页新增最近种子记录，用户可以重新打开、分享、导出、删除或清空最近从 DMHY 下载过的 `.torrent` 文件；删除、清空或超过 20 条淘汰旧记录时会同步尝试清理对应 APP 本地种子文件。
11. 已在种子交接页新增本机兼容清单摘要，按最近实测记录聚合总样本、可用比例、直开/分享/导出后手动导入/magnet/失败次数和优先观察路径，但不上传、不生成官方客户端名单。
12. 已在种子交接页新增兼容报告复制入口、跨设备 Markdown 兼容模板复制入口和跨设备汇总行复制入口，可以把当前设备检测、resolver 候选客户端、本机兼容清单摘要和本机实测记录汇总为纯文本、完整模板或单行表格；其中导出后手动导入路径会依据用户手动实测样本预填状态，作为后续沉淀跨设备兼容清单的原始材料。
13. 已在种子交接页最近种子面板新增“去播放页选择视频”入口，外部 BT 客户端完成真实视频下载后，用户可以回到播放页手动选择本地视频并交给手机播放器。

待确认：

1. 首期是否需要主动推荐或引导安装外部 BT 客户端。
2. 不同 BT 客户端对 magnet、`.torrent`、`ACTION_VIEW`、`ACTION_SEND` 和导出后手动导入的兼容差异是否需要在现有本机兼容清单摘要和兼容报告之外，进一步沉淀为跨设备、跨客户端的固定兼容清单。
...
6. 已在播放首页新增最近视频本机记录，用户可以快速选用最近手动选择过的视频，并可删除单条记录或清空本机记录；删除记录不会删除真实视频文件。
7. 已支持从 DMHY 种子交接成功提示和种子交接页回流播放页，DMHY 来源会在播放页展示“外部 BT 客户端完成真实视频下载后再手动选择”的上下文提示，帮助用户在外部 BT 客户端完成下载后继续手动选择视频。
8. 当前不扫描外部 BT 客户端下载目录，不申请全文件访问权限，不实现内嵌播放器。
...
5. 已新增 `lib/features/subscriptions`，支持从后台订阅面板或 DMHY 搜索结果页保存 DMHY RSS 订阅关键词、选择动画分类或全站范围、手动检查 RSS、前台立即执行一次后台自动检查规则、后台低频自动检查、新命中识别，并在前台展示、刷新和复制最近自动检查摘要与失败原因。
6. 已把订阅自动检查接入前台服务心跳：服务运行时按最小间隔读取关键词、检查 DMHY RSS，把成功或失败摘要写入本地记录，并通过持续通知展示检查摘要；新命中识别只比较“关键词 + 搜索范围 + 最新标题”，不保存完整 RSS 条目列表。
7. 已支持后台通知默认打开 `/?tab=background` 回到后台页；如果最近一次订阅自动检查发现新资源命中且携带最新关键词，则通知点击会直接打开 `/?tab=dmhy&keyword=...&animeOnly=...&dmhySource=backgroundSubscription` 进入 DMHY 搜索页并展示后台订阅命中来源提示；重复命中同一最新标题时回到后台摘要页；当通知已指向 DMHY 搜索页时额外提供“查看资源”通知按钮，通过主 isolate 消息桥请求打开最新 DMHY 搜索；同时提供“查看后台”通知按钮通过主 isolate 消息桥请求打开后台页，以及“停止后台”通知按钮请求停止前台服务。
8. 已支持从订阅关键词、手动检查结果和带最新命中关键词的自动检查摘要跳转到 DMHY 搜索页，并保留动画分类或全站搜索范围；后台页也可以立即执行后台自动检查规则或复制最近自动检查摘要，便于真实设备排查订阅心跳状态。也支持从 DMHY 搜索结果页反向保存当前搜索词为订阅关键词，后续 `.torrent` 下载仍由用户在 DMHY 页显式触发。

后续建议：

1. 继续按 Android 15 `dataSync` 限制评估订阅检查频率，避免把前台服务当作无限运行下载器。
...
1. Flutter 工程骨架。
2. Bangumi 登录、当前用户信息、搜索输入防抖、搜索结果分页、搜索结果 DMHY 搜索联动、条目详情、我的动画收藏分页列表、收藏列表 DMHY 搜索联动、单条收藏读写、动画章节状态同步、章节类型筛选、已加载章节展开查看、章节分页加载更多、批量标记到第 N 话看过，以及当前已加载章节批量设为看过或未收藏。
3. DMHY RSS 搜索并复制/打开磁力链接，资源卡片展示轻量标题元数据标签。
4. Bangumi 条目详情页带标题跳转到 DMHY 自动搜索资源。
5. 按需解析 DMHY 详情页并下载 `.torrent` 种子文件，同时保留最近种子记录以便重试打开、分享、导出、删除、清空或自动淘汰本地种子记录。
6. 通过系统 Intent、分享、导出或复制，把 magnet 或 `.torrent` 种子文件交给外部 BT 客户端。
7. 已提供手动选择本地视频、最近视频回选、单条最近记录删除并调用播放器的入口，并在 DMHY 种子交接成功提示和种子交接页提供前往播放页的闭环入口；APP 不自动追踪外部 BT 客户端的下载结果。
8. 已提供用户显式开启的 Android 前台服务后台常驻入口。
9. 已提供 DMHY RSS 订阅关键词保存、DMHY 搜索结果一键订阅、手动检查、立即后台检查、后台低频自动检查、新命中识别、最近自动检查摘要入口、自动检查摘要复制入口、订阅关键词/检查结果回流 DMHY 搜索入口、后台通知回流入口、新订阅命中通知查看资源按钮、通知查看后台按钮，以及新订阅命中通知直达带来源提示的 DMHY 搜索入口。

复杂过滤、更多批量管理、订阅通知交互、跨设备外部客户端兼容清单固化、公共目录导出和内置下载器可以在基础闭环跑通后逐步补齐；当前已先提供本机兼容清单摘要、导出后手动导入成功样本、可复制报告、Markdown 兼容模板和跨设备汇总行作为沉淀清单的输入材料。

## 关键风险

1. 移动端 Bangumi OAuth `client_secret` 安全策略需要尽早确认。
2. 首期不内置下载器会显著降低复杂度，但也意味着 APP 无法知道外部 BT 客户端的下载进度、完成状态和视频文件路径。
3. 不同外部 BT 客户端对 magnet、`.torrent`、`ACTION_VIEW`、`ACTION_SEND` 和手动导入的兼容性可能不同，需要做好复制、分享和导出兜底。
