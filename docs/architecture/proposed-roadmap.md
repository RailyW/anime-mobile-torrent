...
10. 已接入条目详情页 DMHY 资源搜索联动，使用条目中文名优先生成关键词并跳转到 DMHY 动画分类 RSS 搜索。
11. 章节进度同步、收藏分页加载和批量管理仍是后续工作。
...
1. Flutter 工程骨架。
2. Bangumi 登录、当前用户信息、搜索、条目详情、我的动画收藏列表和单条收藏读写。
...
章节进度、复杂过滤、RSS 自动订阅、外部客户端兼容性清单、公共目录导出和内置下载器可以在基础闭环跑通后逐步补齐。
