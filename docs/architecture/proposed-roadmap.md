...
7. 后台常驻使用用户显式开启的 Android Foreground Service，当前提供持续通知、低频心跳、通知点击默认回到后台页、新订阅命中时通知直达 DMHY 搜索、重复命中回后台摘要页、通知查看后台按钮、通知停止按钮、DMHY 搜索结果一键保存订阅、DMHY 订阅低频自动检查、前台立即执行后台检查规则、新命中识别、前台最近摘要刷新、自动检查摘要复制和订阅结果回流 DMHY 搜索。
...
9. 已提供 DMHY RSS 订阅关键词保存、DMHY 搜索结果一键订阅、手动检查、立即后台检查、后台低频自动检查、新命中识别、最近自动检查摘要入口、自动检查摘要复制入口、订阅关键词/检查结果回流 DMHY 搜索入口、后台通知回流入口、通知查看后台按钮，以及新订阅命中通知直达 DMHY 搜索入口。
