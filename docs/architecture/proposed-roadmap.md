...
9. 已接入当前用户动画收藏列表读取，使用 `GET /v0/users/{username}/collections?subject_type=2`，并在 Bangumi 首页展示收藏预览。
10. 已接入动画本篇章节观看状态同步，使用 `GET /v0/users/-/collections/{subject_id}/episodes?episode_type=0` 读取章节收藏状态，并使用 `PATCH /v0/users/-/collections/{subject_id}/episodes` 更新单集状态。
11. 已接入条目详情页 DMHY 资源搜索联动，使用条目中文名优先生成关键词并跳转到 DMHY 动画分类 RSS 搜索。
12. 收藏分页加载、章节完整分页、批量标记到第 N 话和批量管理仍是后续工作。
...
1. Flutter 工程骨架。
2. Bangumi 登录、当前用户信息、搜索、条目详情、我的动画收藏列表、单条收藏读写和动画本篇章节状态同步。
