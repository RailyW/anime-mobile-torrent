...
7. 后台常驻使用用户显式开启的 Android Foreground Service，当前提供持续通知、低频心跳、通知点击回到后台页、通知停止按钮、DMHY 订阅低频自动检查和前台最近摘要刷新。
...
10. 已接入动画本篇章节观看状态同步，使用 `GET /v0/users/-/collections/{subject_id}/episodes?episode_type=0` 读取章节收藏状态，并使用 `PATCH /v0/users/-/collections/{subject_id}/episodes` 更新单集状态或批量标记到第 N 话看过；详情页支持展开/收起当前已加载章节、加载更多章节，并在保存后按已加载范围刷新进度。
11. 已接入条目详情页 DMHY 资源搜索联动，使用条目中文名优先生成关键词并跳转到 DMHY 动画分类 RSS 搜索。
12. 章节类型筛选和更多批量管理仍是后续工作。
...
1. 首期是否需要进一步从 DMHY HTML 页面展示真实资源大小、种子数、下载数和完成数。
...
7. 已支持后台通知点击直接打开 `/?tab=background` 回到后台页，并提供“停止后台”通知按钮请求停止前台服务。

后续建议：

1. 继续评估是否需要“打开 DMHY”通知按钮，或在通知中根据最新命中资源引导用户回到 DMHY 搜索页。
...
1. Flutter 工程骨架。
2. Bangumi 登录、当前用户信息、搜索、条目详情、我的动画收藏分页列表、单条收藏读写、动画本篇章节状态同步、已加载章节展开查看、章节分页加载更多和批量标记到第 N 话看过。
...
复杂过滤、章节类型筛选、更多批量管理、订阅通知交互、跨设备外部客户端兼容清单、公共目录导出和内置下载器可以在基础闭环跑通后逐步补齐。
