...
7. 后台常驻使用用户显式开启的 Android Foreground Service，当前提供持续通知、低频心跳、DMHY 订阅低频自动检查和前台最近摘要刷新。
...
2. 为后台自动检查摘要增加更细的通知点击跳转，例如直接回到后台订阅面板。
...
9. 已提供 DMHY RSS 订阅关键词保存、手动检查、后台低频自动检查和最近自动检查摘要入口。
