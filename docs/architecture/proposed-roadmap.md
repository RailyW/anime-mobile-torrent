...
9. 已在种子交接页新增本机真实设备兼容实测记录，用户可以手动标记直开成功、分享成功、magnet 兜底成功或交接失败，并把最近记录保存在本机；如果记录错误，可以删除单条实测记录而不清空其他样本。
10. 已在种子交接页新增最近种子记录，用户可以重新打开、分享、导出、删除或清空最近从 DMHY 下载过的 `.torrent` 文件；删除、清空或超过 20 条淘汰旧记录时会同步尝试清理对应 APP 本地种子文件。
11. 已在种子交接页新增本机兼容清单摘要，按最近实测记录聚合总样本、可用比例、直开/分享/magnet/失败次数和优先观察路径，但不上传、不生成官方客户端名单。
12. 已在种子交接页新增兼容报告复制入口、跨设备 Markdown 兼容模板复制入口和跨设备汇总行复制入口，可以把当前设备检测、resolver 候选客户端、本机兼容清单摘要和本机实测记录汇总为纯文本、完整模板或单行表格，作为后续沉淀跨设备兼容清单的原始材料。
13. 已在种子交接页最近种子面板新增“去播放页选择视频”入口，外部 BT 客户端完成真实视频下载后，用户可以回到播放页手动选择本地视频并交给手机播放器。

待确认：

1. 首期是否需要主动推荐或引导安装外部 BT 客户端。
2. 不同 BT 客户端对 magnet、`.torrent`、`ACTION_VIEW`、`ACTION_SEND` 和导出后手动导入的兼容差异是否需要在本机兼容清单摘要和兼容报告之外建立跨设备、跨客户端的兼容性清单。
...
复杂过滤、更多批量管理、订阅通知交互、跨设备外部客户端兼容清单固化、公共目录导出和内置下载器可以在基础闭环跑通后逐步补齐；当前已先提供本机兼容清单摘要、可复制报告、Markdown 兼容模板和跨设备汇总行作为沉淀清单的输入材料。
