...
12. 已在种子交接页新增兼容报告复制入口，可以把当前设备检测、resolver 候选客户端、本机兼容清单摘要和本机实测记录汇总为纯文本，作为后续沉淀跨设备兼容清单的原始材料。
...
复杂过滤、更多批量管理、订阅通知交互、跨设备外部客户端兼容清单固化、公共目录导出和内置下载器可以在基础闭环跑通后逐步补齐；当前已先提供本机兼容清单摘要和可复制报告作为沉淀清单的输入材料。
