...
11. 已在种子交接页新增兼容报告复制入口，可以把当前设备检测、resolver 候选客户端和本机实测记录汇总为纯文本，作为后续沉淀跨设备兼容清单的原始材料。
12. 已在种子交接页最近种子面板新增“去播放页选择视频”入口，外部 BT 客户端完成真实视频下载后，用户可以回到播放页手动选择本地视频并交给手机播放器。

待确认：

1. 首期是否需要主动推荐或引导安装外部 BT 客户端。
2. 不同 BT 客户端对 magnet、`.torrent`、`ACTION_VIEW`、`ACTION_SEND` 和导出后手动导入的兼容差异是否需要在本机兼容报告之外建立跨设备、跨客户端的兼容性清单。
...
复杂过滤、更多批量管理、订阅通知交互、跨设备外部客户端兼容清单固化、公共目录导出和内置下载器可以在基础闭环跑通后逐步补齐。
