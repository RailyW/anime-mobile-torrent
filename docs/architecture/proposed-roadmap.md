...
5. 已支持按用户点击解析 DMHY 详情页 `.torrent` 链接，并把种子文件下载到 APP 专属文档目录的 `dmhy_torrents` 子目录、写入最近种子记录后优先直开外部 BT 客户端，直开失败时自动降级到系统分享面板；交接成功提示可直接回流播放页，由用户在外部 BT 客户端完成视频下载后手动选择本地视频；交接失败或种子下载异常时可从提示条复制 RSS magnet 继续手动处理。
...
7. 已支持从 DMHY 种子交接成功提示和种子交接页回流播放页，帮助用户在外部 BT 客户端完成下载后继续手动选择视频。
