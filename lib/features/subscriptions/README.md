...
`lib/features/subscriptions` 负责 DMHY RSS 订阅检查能力：保存用户显式添加的关键词、按关键词手动检查 DMHY RSS，并在用户显式启动 Android 前台服务后支持低频自动检查。当前模块只做订阅配置、RSS 检查、新命中识别和回流到 DMHY 搜索，不下载 `.torrent` 种子文件，不打开 magnet，也不管理 BT 视频下载。
