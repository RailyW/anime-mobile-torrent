...
- `presentation/dmhy_tab.dart`：搜索 tab（顶部圆角搜索框 + 排序下拉 + “仅动画”开关），提供初始关键词、初始搜索范围和后台订阅命中入口语境自动搜索、后台订阅命中来源提示、回到后台摘要入口、RSS 关键词搜索、搜索结果一键保存为订阅关键词、可折叠的前台筛选面板、字幕组偏好保存/清除与自动套用、字幕语言筛选、最小种子数和排除关键词输入筛选、精简后的资源卡片（标题 + 关键信息标签 + 主种子按钮/打开 magnet/复制 magnet），主种子按钮按设备检测结果动态调整为“打开种子/分享种子/复制磁力”，并在 `.torrent` 下载后写入最近种子记录、执行外部客户端直开/分享兜底、在成功提示中提供带 DMHY 来源语境的播放页回流动作、在失败提示中提供复制 magnet 兜底动作。页面已去掉顶部品牌横幅、能力清单与逐卡的客户端自检说明文字。
