...
`lib/features/dmhy` 负责 DMHY 资源发现：RSS 搜索、最新资源流、资源结果展示、详情页解析、`.torrent` 种子文件下载和外部客户端交接入口。当前已接入 DMHY RSS 关键词搜索和空关键词最新发布读取，支持从 Bangumi 搜索结果、Bangumi 收藏列表、Bangumi 条目详情和 DMHY 订阅检查接收初始关键词自动搜索，订阅回流时可以保留动画分类或全站范围；后台订阅通知回流会携带入口语境，并在搜索结果页展示“来自后台订阅命中”的提示和回到后台摘要的入口；搜索结果页也可以把非空当前关键词和搜索范围保存为 DMHY 订阅关键词，供后台低频检查继续使用；支持 RSS 搜索、详情页解析和种子文件下载请求遇到 429 时按 `Retry-After` 轻量退避并重试一次，支持从 RSS 结果复制/打开 magnet，并可按用户点击解析详情页、下载种子文件到 APP 专属持久目录、写入最近种子记录后优先直开外部 BT 客户端，直开失败时自动降级到系统分享面板；交接成功提示会提供“去播放”动作，带着 DMHY 种子交接来源语境把用户带到播放页手动选择外部 BT 客户端下载好的视频文件，交接失败或种子下载异常提示会提供“复制磁力”兜底动作。资源卡片会读取 `torrent_handoff` 的当前设备检测结果，在用户点击前提示直开、分享导入或无客户端状态，并把主按钮动态调整为“打开种子”“分享种子”或“复制磁力”。资源卡片还会从 RSS 标题和简介中提取字幕组、话数、分辨率、片源、编码、封装格式、字幕说明、归一化字幕语言和文本中的大小标签，并在前台搜索时按需合并 HTML 列表页中的真实大小、種子、下載和完成统计，支持按发布时间、种子数、下载数、完成数或文件大小排序，也支持按字幕组、分辨率、片源、封装格式、视频编码、字幕说明、字幕语言、大小区间、最小种子数和排除关键词进行前台筛选，筛选抽屉会实时显示当前草稿命中数量并提供最小种子快捷阈值；用户可以把当前字幕组保存为本机偏好，后续搜索结果中出现该字幕组时会自动套用筛选，帮助用户更快缩小候选资源。

## 当前包含文件

- `domain/dmhy_filter_preference.dart`：DMHY 前台筛选本机偏好模型，当前保存用户显式选择的字幕组偏好，并提供 JSON 序列化和输入归一化。
- `domain/dmhy_entry_context.dart`：DMHY 首页入口语境和路由构造契约，负责把后台订阅命中来源编码到首页查询参数，并在路由解析后恢复为页面可展示的入口提示。
...
- `data/dmhy_rss_client.dart`：DMHY RSS HTTP 客户端，封装 RSS 请求、空关键词最新发布读取、解析器调用、429 退避和网络错误映射。
...
- `application/dmhy_providers.dart`：DMHY Repository 抽象、RSS 实现、种子文件客户端编排、搜索请求值对象、空关键词最新资源语义、前台 HTML 统计合并、资源排序和 Riverpod 搜索 Provider。
- `presentation/dmhy_tab.dart`：资源 tab（页面内资源顶栏 + DMHY 来源标签 + 顶部圆角搜索框 + “仅动画/全站”分段开关 + 排序 chips），提供空关键词最新资源流、输入防抖 live refresh、初始关键词、初始搜索范围和后台订阅命中入口语境自动搜索、后台订阅命中来源提示、回到后台摘要入口、RSS 关键词搜索、非空搜索结果一键保存为订阅关键词、底部前台筛选抽屉、筛选草稿实时命中数、最小种子数快捷 chip、字幕组偏好保存/清除与自动套用、字幕语言筛选、最小种子数和排除关键词输入筛选、精简后的资源卡片（标题 + 关键信息标签 + 固定种子/下载/完成统计 + 主种子按钮/打开 magnet/复制 magnet），主种子按钮按设备检测结果动态调整为“打开种子/分享种子/复制磁力”，并在 `.torrent` 下载后写入最近种子记录、执行外部客户端直开/分享兜底、在成功提示中提供带 DMHY 来源语境的播放页回流动作、在失败提示中提供复制 magnet 兜底动作。页面已去掉顶部品牌横幅、能力清单与逐卡的客户端自检说明文字。
...
2. 资源 tab 初始进入会读取 DMHY 最新 RSS 用于前台展示；后台自动抓取或自动下载仍然禁止，所有 magnet 打开、`.torrent` 下载和订阅保存都必须由用户显式触发。
3. RSS/HTML 都不是强契约 API，解析代码必须允许字段缺失、格式变化和请求失败。
4. 当前默认使用 `https://dmhy.org/topics/rss/sort_id/2/rss.xml` 读取动画分类最新资源；输入关键词后使用 `?keyword=...` 搜索动画分类，用户可以切换到全站 RSS。
...
9. 资源卡片会根据外部 BT 客户端检测结果调整主按钮文案（打开种子/分享种子/复制磁力）；检测不可用时保留默认“下载种子”交接动作，明确无 `.torrent` 接收路径时把主按钮切换为复制 magnet 兜底。本次 UI 重构去掉了逐卡的客户端自检说明文字，但主按钮的自适应行为保持不变。
...
13. 后台订阅通知进入 DMHY 时只携带来源语境和搜索参数；来源提示不会改变 RSS 请求、筛选结果或种子交接行为，用户手动提交新搜索或切换搜索范围后会恢复为普通入口。
