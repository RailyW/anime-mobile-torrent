...
- `home_screen.dart`：Material 3 首页壳，使用底部 `NavigationBar` 和 `IndexedStack` 切换功能模块，并支持通过路由参数初始选中 DMHY 或后台标签页；DMHY 跳转可以额外注入搜索关键词和动画分类/全站搜索范围；右上角设置按钮会打开 Bangumi OAuth 本机配置页，并在设置页返回后刷新 Bangumi 授权运行期状态。
