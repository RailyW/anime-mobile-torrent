...
- `presentation/torrent_handoff_tab.dart`：种子交接首页入口，明确展示 DMHY 内已接入的 magnet 打开、`.torrent` 下载直开、分享兜底能力、当前设备外部客户端检测结果和候选客户端、最近种子面板、本机兼容清单摘要、本机兼容实测记录面板、兼容报告复制入口、跨设备 Markdown 兼容模板复制入口、跨设备汇总行复制入口、外部 BT 客户端兼容自检步骤和失败时处理路径；最近种子面板支持单条打开、分享、导出、删除、整体清空，并提供跳转播放页手动选择本地视频的入口。
