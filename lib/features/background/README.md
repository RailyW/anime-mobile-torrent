...
`lib/features/background` 负责用户显式开启的 Android 后台常驻能力。当前使用成熟 Flutter 插件 `flutter_foreground_task` 接入 Android Foreground Service，提供持续通知、低频心跳、启动/停止/刷新状态入口，并在页面中承载 DMHY RSS 订阅检查面板；当用户已经保存订阅关键词且显式启动前台服务后，后台任务会按低频间隔自动检查 DMHY RSS 并更新持续通知。
...
5. 当前不会在后台请求 Bangumi，不下载 `.torrent`，不管理 BT 视频下载，不在开机后自动启动；DMHY 订阅自动检查仅在用户已经保存关键词并显式启动服务后按间隔运行。
