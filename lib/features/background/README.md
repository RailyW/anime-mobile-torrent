...
`lib/features/background` 负责用户显式开启的 Android 后台常驻能力。当前使用成熟 Flutter 插件 `flutter_foreground_task` 接入 Android Foreground Service，提供持续通知、低频心跳、启动/停止/刷新状态入口，并在页面中承载 DMHY RSS 订阅检查面板；当用户已经保存订阅关键词且显式启动前台服务后，后台任务会按低频间隔自动检查 DMHY RSS，写入最近一次成功或失败摘要，并更新持续通知。

## 当前包含文件

- `domain/background_residency_state.dart`：后台常驻服务状态枚举、状态快照和 UI 状态模型。
- `data/background_residency_repository.dart`：后台常驻服务仓库接口、`flutter_foreground_task` 实现、前台服务初始化、通知权限请求、启动/停止/刷新逻辑，以及后台 isolate 使用的 `TaskHandler`；`TaskHandler` 只调用 `subscriptions` 模块的自动检查服务并更新通知，不直接实现 RSS 解析。
