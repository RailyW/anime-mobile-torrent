...
- `app_router.dart`：应用路由表 Provider，注册首页路由、本地播放页、种子工具页、后台与订阅页、Bangumi OAuth 设置页、Bangumi 条目搜索页和 Bangumi 条目详情命名路由；首页采用“Bangumi / 资源 / 我的”三段式底部导航，Bangumi 条目详情路由使用淡入上移的自定义页面转场以承接封面 Hero 动画。首页路由支持 `tab=dmhy&keyword=...&animeOnly=...&dmhySource=...` 用于从 Bangumi 条目详情、订阅检查或后台订阅通知跳转到资源 tab 并自动搜索，并在后台订阅命中时透传来源提示；历史深链 `tab=playback&source=dmhyTorrent`、`tab=torrent`、`tab=background` 会落到“我的”tab 并自动推入对应子页面（带来源提示的播放页、种子工具页、后台与订阅页），保证后台通知与跨模块跳转的目标功能仍可直达。
