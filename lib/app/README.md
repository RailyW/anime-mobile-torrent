...
- `app_router.dart`：应用路由表 Provider，目前注册首页路由和 Bangumi 条目详情命名路由；首页路由支持 `tab=dmhy&keyword=...` 查询参数用于从 Bangumi 条目详情跳转到 DMHY 自动搜索，也支持 `tab=background` 用于后台常驻通知点击后直接进入后台页。
