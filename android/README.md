...
- `app/src/main/AndroidManifest.xml`：声明网络权限、启动 Activity、Flutter embedding 和外部 BT 客户端查询能力。
...
- `build.gradle.kts`、`settings.gradle.kts`、`gradle.properties`：Android Gradle 构建配置。
- `gradle/wrapper/`：Gradle Wrapper 配置。

## 设计边界

1. 首期 Android 原生侧只承载 Flutter UI、网络权限和外部应用交接查询声明。
