...
- `app/src/main/AndroidManifest.xml`：声明网络权限、前台服务权限、前台服务类型权限、Android 13+ 通知权限、启动 Activity、Flutter embedding、`flutter_foreground_task` 服务、Bangumi 授权页外部浏览器兜底所需的 `https` 查询能力、外部 BT 客户端查询能力和 `video/*` 播放器查询能力。
...
1. 首期 Android 原生侧承载 Flutter UI、网络权限、WebView 授权页所需运行环境、WebView 授权失败时的外部浏览器查询声明、用户显式启动的前台服务声明、Android 13+ 通知权限声明、外部 BT 客户端交接查询声明、外部播放器查询声明、外部 BT 客户端 resolver 检测通道和用户主动触发的 `.torrent` 导出通道。
...
Bangumi 开发者后台中的 redirect URI 应与 Flutter 侧默认值保持一致：`com.railyw.anime_mobile_torrent://oauth/bangumi`。Bangumi 当前授权完成后会生成 `https://bgm.tv/oauth/<redirect_uri>?code=...`，因此 APP 使用 `webview_flutter` 在页面导航阶段截获这个 HTTPS 代理回调，不再依赖 Android 自定义 scheme intent-filter 或 AppAuth 回跳 Activity。若模拟器或设备的 Android WebView renderer 崩溃，Flutter 授权页会通过 `url_launcher` 改用系统浏览器打开同一授权地址，并让用户把最终回调地址或 code 粘贴回 APP，因此 manifest 在 Android 11+ 包可见性查询中声明了 `https` VIEW 能力。双斜杠形态仍然保留，用于避免旧单斜杠 URI 被 Bangumi 拼接成更难识别的站内路径。
