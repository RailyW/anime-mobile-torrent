...
- `widget_test.dart`：首页烟测，验证 APP 可以加载并切换 Bangumi、DMHY、种子交接和播放四个主要模块；同时用 fake repository 验证 Bangumi 搜索结果渲染、Bangumi 搜索结果进入条目详情页、Bangumi 详情页跳转 DMHY 自动搜索、DMHY RSS 搜索结果渲染和 DMHY 种子文件下载后外部客户端交接。
