...
- `torrent_handoff_test.dart`：Torrent 交接模型单元测试，验证通用种子文件模型、文件大小格式化、交接结果中文提示、种子导出结果中文提示、Android 导出通道状态映射、外部 BT 客户端能力检测结果与候选应用解析、本机兼容实测记录序列化、本机兼容清单摘要聚合、兼容报告纯文本生成、最近种子记录序列化、`SharedPreferences` 持久化仓库，以及单条删除、整体清空和超过 20 条自动淘汰时的本地种子文件清理。
